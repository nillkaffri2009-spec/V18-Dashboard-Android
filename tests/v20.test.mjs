...
test('role routes use shared app, control selector never contains monitor and polling is 2 seconds',async()=>{
 ...
 const app=await fs.readFile(new URL('../worker/public/app.js',import.meta.url),'utf8');assert(app.includes('function monitorInches()'));assert(app.includes("return Number.isFinite(q)&&q>=24&&q<=100?q:43"));assert(app.includes("const factor=Math.min(1.18"));assert(app.includes('setInterval(syncData,2000)'));assert(app.includes('setInterval(checkBuildVersion,2000)'));assert(app.includes('rosterScrollRatio'));assert(app.includes('employeeScrollRatio'));assert(html.includes('manualRefresh'));assert(html.includes('v=20.7-final16'));assert(!app.includes("data.source==='computer'&&!active"));
 ...
 const health=await (await worker.fetch(new Request('https://test/api/health'),env)).json();assert.equal(health.version,'20.7-cloud');assert.equal(health.assetVersion,'20.7-final16');
});
