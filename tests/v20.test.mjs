...
test('PC and phone can repeatedly reclaim; monitor stays read only; stale owner rejected',async()=>{
 ...
 phone=await send('phone','phone',{action:'update',epoch:phone.epoch,view:{...phone.view,department:'21120',page:2,scrollRatio:0.62,rosterScrollRatio:0.44,employeeScrollRatio:0.33,deptTableScrollRatio:0.55,kpiTableScrollRatio:0.77}});
 const tv=await send('monitor','tv');assert.equal(tv.view.department,'21120');assert.equal(tv.view.scrollRatio,0.62);assert.equal(tv.view.rosterScrollRatio,0.44);assert.equal(tv.view.employeeScrollRatio,0.33);assert.equal(tv.view.deptTableScrollRatio,0.55);assert.equal(tv.view.kpiTableScrollRatio,0.77);
...
test('role routes use shared app, control selector never contains monitor and polling is 2 seconds',async()=>{
 ...
 const css=await fs.readFile(new URL('../worker/public/additions.css',import.meta.url),'utf8');assert(css.includes('V20.7 FINAL 16 — 43-inch BALANCED TV PROFILE'));assert(css.includes('--tv-body:20px'));assert(css.includes('.monitor-mode .roster-table{font-size:var(--tv-body)'));
 const app=await fs.readFile(new URL('../worker/public/app.js',import.meta.url),'utf8');assert(app.includes('function monitorInches()'));assert(app.includes("return Number.isFinite(q)&&q>=24&&q<=100?q:43"));assert(app.includes("const factor=Math.min(1.18"));assert(app.includes("const autoTake=url.searchParams.get('take_control')==='1'||role==='phone'"));assert(app.includes("if(role==='phone')await action('claim')"));assert(app.includes('setInterval(syncData,2000)'));assert(app.includes('setInterval(checkBuildVersion,2000)'));assert(app.includes('rosterScrollRatio'));assert(app.includes('employeeScrollRatio'));assert(html.includes('manualRefresh'));assert(html.includes('v=20.8-final18'));assert(!app.includes("data.source==='computer'&&!active"));
 ...
 const health=await (await worker.fetch(new Request('https://test/api/health'),env)).json();assert.equal(health.version,'20.8-cloud');assert.equal(health.assetVersion,'20.8-final18');
});
