...
function monitorInches(){
 const q=Number(url.searchParams.get('tv'));
 return Number.isFinite(q)&&q>=24&&q<=100?q:43;
}
function applyMonitorTypography(){
 if(role!=='monitor'){
  for(const k of ['--tv-body','--tv-small','--tv-title','--tv-nav','--tv-heading','--tv-kpi','--tv-clock'])document.body.style.removeProperty(k);
...
window.addEventListener('resize',()=>applyMonitorTypography(),{passive:true});
